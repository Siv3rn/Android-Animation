...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    // check which animation to be played
    // 0 for exit and 1 for enter
    ...
    int[] exitDelay = {80,40,120,0,120,160};
    ...
    private void calculatePentagonVertices(int radius, int rotation) {
        ...
        for(int i = 0; i < NUM_OF_SIDES; i++){
            pentagonVertices[i] = new Point((int) (radius * Math.cos(rotation + i * 2 * Math.PI/NUM_OF_SIDES))
                    + centerX, (int) (radius * Math.sin(rotation + i * 2 * Math.PI / NUM_OF_SIDES)) + centerY - 100);
        }
